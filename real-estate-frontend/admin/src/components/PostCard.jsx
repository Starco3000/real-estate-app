...
function PostCard({ data, handleEvent, isUpdate }) {
  ...
  return (
    <Link to={`/admin/${data._id}`} className='block'>
      <div className='w-full h-[200px] font-lexend font-normal text-sm flex flex-row overflow-hidden'>
        <div
          className={`w-full h-[200px] bg-white border-[1px] border-gray-300 flex flex-row gap-x-4 overflow-hidden ${
            isUpdate ? 'rounded-l-lg' : 'rounded-lg'
          } `}
        >
          <img
